...
let current_operation: Operation = Operation.NONE;
 ...
 */
function handleNumberClick(event: MouseEvent) {
    if (pending_input) {
        display_value = 0;
        pending_input = false;
    }

    let num = parseInt((event.target as HTMLButtonElement).textContent);
    if (!Number.isNaN(num)){
...
* @param event - The click event object
*/
function handleOperationClick(event: MouseEvent) {
    let button_id: string = (event.target as HTMLButtonElement).id;
 ...
 */
function performOperation(op: Operation, val1: number, val2: number): number { 
    if (op === Operation.ADD){
        return val1 + val2;
    }
 ...
 */
function appendToDisplayValue(num: number): void{
    if(Math.abs(num) < 10){
